...
fn read_shunt_calibration_works() {
    ...
    let spi_expectations = [
        ...
        SPITransaction::transfer_in_place(vec![0x09, 0x00, 0x00], vec![0x00, 0x10, 0x00]),
...
fn write_shunt_calibration_works() {
    ...
    let spi_expectations = [
        SPITransaction::transaction_start(),
        SPITransaction::write_vec(vec![0x08, 0x10, 0x00]),
        SPITransaction::transaction_end(),
    ];
    let spi = SPIMock::new(&spi_expectations);
    let mut ina229 = INA229::new(spi);
...
fn set_shunt_calibration_accepts_max_valid_value() {
    ...
    let spi_expectations = [
        SPITransaction::transaction_start(),
        SPITransaction::write_vec(vec![0x08, 0x7F, 0xFF]),
        SPITransaction::transaction_end(),
    ];
    let spi = SPIMock::new(&spi_expectations);
    let mut ina229 = INA229::new(spi);
